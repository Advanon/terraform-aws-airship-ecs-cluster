...
resource "aws_autoscaling_schedule" "evening_shutdown" {
  count                  = "${lookup(var.cluster_properties, "asg_schedule_enabled")}"
  scheduled_action_name  = "${aws_autoscaling_group.this.name}_workday_evening_shutdown"
  min_size               = 0
  max_size               = 0
  desired_capacity       = 0
  recurrence             = "${lookup(var.cluster_properties, "asg_schedule_shutdown_recurrence")}"
  autoscaling_group_name = "${aws_autoscaling_group.this.name}"
}

resource "aws_autoscaling_schedule" "morning_startup" {
  count                  = "${lookup(var.cluster_properties, "asg_schedule_enabled")}"
  scheduled_action_name  = "${aws_autoscaling_group.this.name}_workday_morning_startup"
